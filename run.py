...
def perform_evaluation(model, builder, eval_steps, ckpt, strategy, topology):
  ...
  #                                         strategy, topology)
  # if is_training:
  #   with open('spectrograms/train_data.pickle','rb') as f:
  #     ds=pickle.load(f)
  # else:
  #   with open('spectrograms/val_data.pickle','rb') as f:
  #     ds=pickle.load(f)
  with open('spectrograms/augmented_list_batch_wise.pickle','rb') as f:
      ds=pickle.load(f)
...
def main(argv):
  ...
  builder=None # Dummy
  num_train_examples = 44 #(80 % of 55)
  num_eval_examples = 11  #(20% of 55)
  num_classes = 3
  ...
  if FLAGS.mode == 'eval':
    ...
    with strategy.scope():
      ...
      tf.saved_model.save(model, 'pre-trained_model/')
